...
def _resolveWebsiteObj(domainName):
    """Root domains live in Websites; subdomains live in ChildDomains and
    share their master site's linux user (externalApp)."""
    try:
        return Websites.objects.get(domain=domainName)
    except Websites.DoesNotExist:
        return ChildDomains.objects.get(domain=domainName).master


def _resolveHomePath(domainName):
    """Root domains live at /home/<domain>; subdomains live under their
    master's public_html, per ChildDomains.path."""
    if Websites.objects.filter(domain=domainName).exists():
        return '/home/%s' % domainName
    childDomain = ChildDomains.objects.get(domain=domainName)
    return os.path.join(childDomain.path, domainName)
...
"""
    ...
    def changeOwner(self, path):
        try:
            ...
            website = _resolveWebsiteObj(domainName)
    ...
    def listForTable(self):
        try:
            ...
            try:
                ...
                pathCheck = _resolveHomePath(domainName)
    ...
    def createNewFile(self):
        try:
            ...
            try:
                domainName = self.data['domainName']
                website = _resolveWebsiteObj(domainName)
                homePath = _resolveHomePath(domainName)
    ...
    def deleteFolderOrFile(self):
        try:
            ...
            try:
                ...
                website = _resolveWebsiteObj(domainName)
                self.homePath = _resolveHomePath(domainName)
    ...
    def writeFileContents(self):
        try:
            ...
            finalData['status'] = 1
            try:
                self.data['home'] = _resolveHomePath(self.data['domainName'])
    ...
    def upload(self):
        try:
            ...
            domainName = self.data['domainName']
            try:
                pathCheck = _resolveHomePath(self.data['domainName'])
                website = _resolveWebsiteObj(domainName)
