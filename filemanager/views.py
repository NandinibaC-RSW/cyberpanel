...
from .filemanager import FileManager as FM, _resolveHomePath
...
def loadFileManagerHome(request,domain):
    try:
        ...
        if Websites.objects.filter(domain=domain).exists() or ChildDomains.objects.filter(domain=domain).exists():
            ...
            if ACLManager.checkOwnership(domain, admin, currentACL) == 1:
                ...
                context = {
                    'domainName': domain,
                    'homePath': _resolveHomePath(domain),
                    'ipAddress': ipAddress,
