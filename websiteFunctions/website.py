...
""" % (self.domain, website.externalApp)
    ...
    def cloneWithToken(self, userID=None, data=None):
        try:
            currentACL = ACLManager.loadedACL(userID)
            admin = Administrator.objects.get(pk=userID)
            ...
            gitHttpUrl = data['gitHttpUrl'].strip()
            ...
            if self.folderCheck():
                ...
                return HttpResponse(json.dumps({'status': 0, 'error_message': 'Folder is outside the website home directory.'}))

            ## Validate the HTTPS URL strictly - only https://<host>[:port]/<path>,
            ## and reject anything containing shell metacharacters or credentials
            ## already embedded in the URL (those must go through the token field).
            urlMatch = re.match(r'^https://([a-zA-Z0-9.-]+)(?::[0-9]{1,5})?/[\w.\-/]+?(?:\.git)?/?$', gitHttpUrl)
            if not urlMatch:
                return HttpResponse(json.dumps({'status': 0, 'error_message': 'Invalid HTTPS git URL. Use the form https://host/user/repo.git'}))

            gitHostDomain = urlMatch.group(1)

            if not validators.domain(gitHostDomain):
                return HttpResponse(json.dumps({'status': 0, 'error_message': 'Invalid characters in git host.'}))

            if ACLManager.validateInput(gitUsername):
                pass
            else:
                return HttpResponse(json.dumps({'status': 0, 'error_message': 'Invalid characters in username.'}))

            ## Personal access tokens are alnum plus common separators - never
            ## allow this into a shell command unvalidated.
            if not re.fullmatch(r'[A-Za-z0-9_\-.]+', gitToken):
                return HttpResponse(json.dumps({'status': 0, 'error_message': 'Invalid characters in token.'}))

            ## NOTE: ProcessUtilities.executioner runs commands via
            ## `sudo -u <user> <command>` tokenized with shlex.split and NO shell
            ## (shell=False under the hood even when a user is given, and the
            ## worker process cannot reach the root-owned lscpd IPC path with
            ## shell=True either) - so `&&`, `>`, `|` are NOT interpreted as
            ## shell operators, they are passed as literal argv tokens and
            ## silently do nothing useful. Every command below is therefore a
            ## single plain command with no shell metacharacters, matching how
            ## the rest of this file already calls executioner. Multi-step
            ## operations are issued as separate executioner calls instead of
            ## chaining with &&. Also: os.path.exists/os.listdir() run as this
            ## worker process's own limited user and cannot see inside
            ## /home/<site>, so folder/file existence checks are done the same
            ## way - as a command run as the site's own linux user.

            existingEntries = ProcessUtilities.outputExecutioner(
                'find %s -mindepth 1 -maxdepth 1' % self.folder, self.externalAppLocal).strip()
            if existingEntries:
                return HttpResponse(json.dumps({'status': 0, 'error_message': 'Target folder is not empty.'}))

            ## Per-domain credential store, owned by the site's linux user and
            ## chmod 600 - the token never appears in argv (ps aux) or shell
            ## history, only in this permission-locked file.
            credDir = '/home/%s/.git-credentials-store' % (self.masterDomain)
            ProcessUtilities.executioner('mkdir -p %s' % credDir, self.externalAppLocal)
            ProcessUtilities.executioner('chmod 700 %s' % credDir, self.externalAppLocal)

            safeDomainFile = re.sub(r'[^A-Za-z0-9_.-]', '_', self.domain)
            credFile = '%s/%s.txt' % (credDir, safeDomainFile)

            ## Written via a single python3 -c argv (no shell redirection needed)
            ## - gitUsername/gitToken/gitHostDomain are already validated above
            ## to only ever contain alnum plus [._-], so repr() quoting here is
            ## just defense in depth, not load-bearing for safety.
            credLine = 'https://%s:%s@%s\n' % (gitUsername, gitToken, gitHostDomain)
            writeCmd = "python3 -c \"open(%r, 'w').write(%r)\"" % (credFile, credLine)
            ProcessUtilities.executioner(writeCmd, self.externalAppLocal)
            ProcessUtilities.executioner('chmod 600 %s' % credFile, self.externalAppLocal)

            ## Clone with the credential helper pointed at that file - git resolves
            ## auth from the file, so the token is never part of the clone command.
            command = 'git clone --config credential.helper="store --file=%s" %s %s' % (
                credFile, gitHttpUrl, self.folder)
            commandStatus = ProcessUtilities.outputExecutioner(command, self.externalAppLocal)

            cloneOk = ProcessUtilities.executioner('test -d %s/.git' % self.folder, self.externalAppLocal)
            if not cloneOk:
                return HttpResponse(json.dumps({'status': 0, 'error_message': 'Clone failed: ' + str(commandStatus)}))

            ## Persist the credential helper on the repo itself so the existing
            ## gitPull/gitPush endpoints authenticate automatically afterwards.
            command = 'git -C %s config credential.helper "store --file=%s"' % (self.folder, credFile)
            ProcessUtilities.executioner(command, self.externalAppLocal)
