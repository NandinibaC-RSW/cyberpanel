        ...
        try:
            ...
            if aliasDomain is None:
                ...
                try:
                    ...
                    # --- PATCHED: issue directly from PRODUCTION Let's Encrypt ---
                    # Previous code issued a STAGING cert first (--staging), which wrote an
                    # acme.sh conf locked to the staging/test CA endpoint. The subsequent
                    # production re-issue then silently failed, so certs were never installed
                    # and sites fell back to CyberPanel's self-signed default. Issuing
                    # production directly avoids the stale-staging-config trap.
                    ...
                    if result.returncode == 0:
                        # Install the certificate to the desired location
                        ...
                        if install_result.returncode == 0:
                            ...
                            # Log install failure (previously silent)
                            logging.CyberCPLogFileWriter.writeToFile(
                                "acme.sh install-cert failed for %s: %s" % (virtualHostName, install_result.stderr))
                    else:
                        # Log issuance failure (previously silent — root cause of self-signed fallback)
                        logging.CyberCPLogFileWriter.writeToFile(
                            "acme.sh production issue failed for %s: %s" % (virtualHostName, result.stderr))
                    return 0
                except Exception as e:
        ...
        if os.path.exists(existingCertPath):
            ...
            if os.path.exists(acmePath):
                ...
                if is_expired:
                    ...
                    command = f'{acmePath} --issue {renewal_domains} --webroot /usr/local/lsws/Example/html --force --server letsencrypt'
                else:
